function [FEMatrices] = MatricesParts_fluid_sp_SAFE_cubic(CompStruct,...
...
%
% MatricesParts_fluid_sp_SAFE_cubic M-file      
%      MatricesParts_fluid_sp_SAFE_cubic, by itself, 
...
%      Ready and can handle inhomogeneous fluid case, but currently is used for the homogeneous case 
%       (more of an issue with the parameter input)
%
%   [T.Zharnikov SMR v0.3_09.2014]
%
% function [MMatrix_d,K1Matrix_d,K2Matrix_d,K3Matrix_d] = ...
%        MatricesParts_fluid_sp_SAFE_cubic(DElements, DEMeshProps, DNodes,CompStruct)
%
%  Inputs - 
%
...
%###############################################################################
%
%   Code for MatricesParts_fluid_sp_SAFE_cubic
%
%###############################################################################
...
%     el_y_nodes_full = MeshNodes(2,MeshTri(1:10,ii_el));  
    ...
    VarRows = repmat(VarVecArr,1,10);
    VarCols = reshape(repmat(VarVecArr,10,1),1,[]);
