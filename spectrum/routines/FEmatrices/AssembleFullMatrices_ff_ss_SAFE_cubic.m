function [FEMatrices,FullMatrices] = ...
        AssembleFullMatrices_ff_ss_SAFE_cubic(CompStruct,BasicMatrices,FEMatrices,FullMatrices,ii_int,ii_d1,ii_d2)
% function [Fluid_bound_matrix,Solid_bound_matrix]=Fluid_solid_boundary_cubic( p,e,s,bnd_arr,Mud_subdomain)
%         %p nodes e edges s CompStruct, bnd_arr Mud_subdomain
        
%   Part of the toolbox for solving problems of wave propagation
% in arbitrary anisotropic inhomogeneous waveguides.
% For details see User manual 
% and comments to the main script gen_aniso.m
%
% MatricesParts_HTTI_sp_SAFE_cubic M-file      
%      MatricesParts_HTTI_sp_SAFE_cubic, by itself, 
%      prepares the blocks of the full stiffness and mass matrices 
%      for the particular domain.
%      Sparse matrix representation is used for speed and storage (the suggestion of Denis Syresin).
%      Ready and can handle inhomogeneous anisotropic solid case, but currently is used for the homogeneous case 
%       (more of an issue with the parameter input)
%
%   [T.Zharnikov SMR v0.3_09.2014]
%
% function [MMatrix_d,K1Matrix_d,K2Matrix_d,K3Matrix_d] = ...
%        MatricesParts_HTTI_sp_SAFE_cubic(DElements, DEMeshProps, DNodes,CompStruct)
%
%  Inputs - 
%
%       BasicMatrices - structure containing the basic matrices (differentiation, etc.);
%
%       CompStuct - structure containing the parameters of the model;
%
%       ii_l - number of the layer, for which the basic matrices are
%              assembled
%
%  Outputs -
%
%       L_parts - matrix representation for the governing operator
%
%       Srr_parts - matrix representation for the stress (s_rr) operator
%
%       D_parts - matrix representation for the displacement operator
%
%       M_parts - matrix representation for the left-hand side of the governing
%                   equations' operator
%
%  M-files required-
%
% Last Modified by Timur Zharnikov SMR v0.3_09.2014

%###############################################################################
%
%   Code for MatricesParts_HTTI_sp_SAFE_cubic
%
%###############################################################################

%===============================================================================
% Initialization
%===============================================================================

% D1NodesNum = size(FEMatrices.DNodes{ii_d1},2);
% D1FEVarNum = CompStruct.Data.DVarNum(ii_d1)*D1NodesNum;
% D2NodesNum = size(FEMatrices.DNodes{ii_d2},2);
% D2FEVarNum = CompStruct.Data.DVarNum(ii_d2)*D2NodesNum;
% % create zero sparse matrix for the 
% % fluid-solid boundary condition matrix
% FEMatrices.ZeroD12 = sparse(BFEVarNum1,BFEVarNum2);
% FEMatrices.ZeroD21 = sparse(BFEVarNum1,BFEVarNum2);

%===============================================================================
% Assembling full matrices by adding the mass and stiffness matrices of the
% next block
% and by inserting the matrices, which are responsible for the boundary
% conditions
%===============================================================================

% % example of script for inserting the new matrix blocks using sparse
% % matrices procedures (potentially can increase the speed)
% FullMatrices.K1Matrix = sparse(CompStruct.Data.DVarNum{N_domain}(2),CompStruct.Data.DVarNum{N_domain}(2));
% [ DVecRepRow, DVecRepCol, DVecRepV ] = find(K1Matrix_d{ii_d});
% DVecRepRow = DVecRepRow + CompStruct.Data.DVarNum{ii_d}(1) - 1;
% DVecRepCol = DVecRepCol + CompStruct.Data.DVarNum{ii_d}(1) - 1;
% DSize = size(K1Matrix_d{ii_d});
% K1Matrix = K1Matrix + sparse(DVecRepRow, DVecRepCol, DVecRepV, DSize(1), DSize(2));

% adding the matrices for the next block
FullMatrices.K1Matrix = blkdiag(FullMatrices.K1Matrix,FEMatrices.K1Matrix_d{ii_d2});
FullMatrices.K2Matrix = blkdiag(FullMatrices.K2Matrix,FEMatrices.K2Matrix_d{ii_d2});
FullMatrices.K3Matrix = blkdiag(FullMatrices.K3Matrix,FEMatrices.K3Matrix_d{ii_d2});
FullMatrices.MMatrix = blkdiag(FullMatrices.MMatrix,FEMatrices.MMatrix_d{ii_d2});
FullMatrices.PMatrix = blkdiag(FullMatrices.PMatrix,FEMatrices.PMatrix_d{ii_d2});

%===============================================================================
% Sum the row and columns, which correspond to the contributions of the
% identical nodes
%
% Prepare the full matrices for the collapse when removing redundant rows
% and columns
%===============================================================================

%===============================================================================
% Adjust the list of variables (remove duplicates, if any, etc.)
% and the positions of the variables in the full vector of variables
%===============================================================================

% [FEMatrices.BNodesFull{ii_int}] = sort(unique(BNodesFull));
FEMatrices.DNodesRem{ii_d2} = [ FEMatrices.DNodesRem{ii_d2} FEMatrices.BNodesFull{ii_int} ];
FEMatrices.DNodesRem{ii_d2} = sort(unique(FEMatrices.DNodesRem{ii_d2}));
RemoveNodesPos = ismember(FEMatrices.DNodes{ii_d2},FEMatrices.DNodesRem{ii_d2});
FEMatrices.DNodesComp{ii_d2} = FEMatrices.DNodes{ii_d2}(~RemoveNodesPos);

% finding positions of the rows, which correspond to the nodes, 
% which will be removed
% NB! Nodes in DNodes and BNodesFull are ordered! 
% It is important condition for this script to work.

% AddNodesPos = find(ismember(FEMatrices.DNodes{ii_d1},FEMatrices.BNodesFull{ii_int}));
% RemoveNodesPos = find(ismember(FEMatrices.DNodes{ii_d2},FEMatrices.BNodesFull{ii_int}));
% 
% VarArr = [1:CompStruct.Data.DVarNum(ii_d1)]';
% sizeVarArr = size(AddNodesPos);
% LargeVarArr = repmat(VarArr,1,sizeVarArr(2));
% AddVarPos = CompStruct.Data.DVarNum(ii_d1)*( AddNodesPos - 1 );
% AddVarPos = repmat(AddVarPos,CompStruct.Data.DVarNum(ii_d1),1);
% AddVarPos = AddVarPos + LargeVarArr;
% AddVarPos = BasicMatrices.Pos{ii_d1}(1) - 1 + reshape(AddVarPos,1,[]);
% 
% VarArr = [1:CompStruct.Data.DVarNum(ii_d2)]';
% sizeVarArr = size(RemoveNodesPos);
% LargeVarArr = repmat(VarArr,1,sizeVarArr(2));
% RemoveVarPos = CompStruct.Data.DVarNum(ii_d2)*( RemoveNodesPos - 1 );
% RemoveVarPos = repmat(RemoveVarPos,CompStruct.Data.DVarNum(ii_d2),1);
% RemoveVarPos = RemoveVarPos + LargeVarArr;
% RemoveVarPos = BasicMatrices.Pos{ii_d2}(1) - 1 + reshape(RemoveVarPos,1,[]);

BNodesFullSize = size(FEMatrices.BNodesFull{ii_int});
BNodesFullSize = BNodesFullSize(2);
AddVarPos = [];
RemoveVarPos = [];
for ii_n = 1:BNodesFullSize
    D1NodePos = find(ismember(FEMatrices.DNodes{ii_d1},FEMatrices.BNodesFull{ii_int}(ii_n)));
    D2NodePos = find(ismember(FEMatrices.DNodes{ii_d2},FEMatrices.BNodesFull{ii_int}(ii_n)));
    AddVarPos(( CompStruct.Data.DVarNum(ii_d1)*( ii_n - 1 ) + 1 ):...
        (CompStruct.Data.DVarNum(ii_d1)*ii_n)) = ...
        BasicMatrices.Pos{ii_d1}(1) - 1 + ...
        [ (CompStruct.Data.DVarNum(ii_d1)*( D1NodePos - 1 ) + 1 ):...
        (CompStruct.Data.DVarNum(ii_d1)*D1NodePos) ];
    RemoveVarPos(( CompStruct.Data.DVarNum(ii_d2)*( ii_n - 1 ) + 1 ):...
        (CompStruct.Data.DVarNum(ii_d2)*ii_n)) = ...
        BasicMatrices.Pos{ii_d2}(1) - 1 + ...
        [ (CompStruct.Data.DVarNum(ii_d2)*( D2NodePos - 1 ) + 1 ):...
        (CompStruct.Data.DVarNum(ii_d2)*D2NodePos) ];
end;

% identify nodes and variables, which will be removed from the full matrices
% and which values should be taken from other nodes (duplicate nodes) and variables
% after computing the eigenvectors of the reduced matrices

FEMatrices.DTakeFromVarPos{ii_d2} = [ FEMatrices.DTakeFromVarPos{ii_d2} AddVarPos ];
FEMatrices.DPutToVarPos{ii_d2} = [ FEMatrices.DPutToVarPos{ii_d2} RemoveVarPos ];

% FEMatrices.RemoveVarPos{ii_d2} = RemoveVarPos;

% for ii = 1:CompStruct.Data.DVarNum(ii_d1)
%     AddVarPos = BasicMatrices.Pos{ii_d1}(1) - 1 + ii + ...
%         CompStruct.Data.DVarNum(ii_d1)*( AddNodesPos - 1 );
%     RemoveVarPos = BasicMatrices.Pos{ii_d2}(1) - 1 + ii + ...
%         CompStruct.Data.DVarNum(ii_d2)*( RemoveNodesPos - 1 );
% end;

%===============================================================================
% Sum the rows and columns, which correspond to the coincident nodes
% in both domains
%===============================================================================

FullMatrices.K1Matrix(AddVarPos,:) = ...
    FullMatrices.K1Matrix(AddVarPos,:) + FullMatrices.K1Matrix(RemoveVarPos,:);
FullMatrices.K1Matrix(:,AddVarPos) = ...
    FullMatrices.K1Matrix(:,AddVarPos) + FullMatrices.K1Matrix(:,RemoveVarPos);

FullMatrices.K2Matrix(AddVarPos,:) = ...
    FullMatrices.K2Matrix(AddVarPos,:) + FullMatrices.K2Matrix(RemoveVarPos,:);
FullMatrices.K2Matrix(:,AddVarPos) = ...
    FullMatrices.K2Matrix(:,AddVarPos) + FullMatrices.K2Matrix(:,RemoveVarPos);

FullMatrices.K3Matrix(AddVarPos,:) = ...
    FullMatrices.K3Matrix(AddVarPos,:) + FullMatrices.K3Matrix(RemoveVarPos,:);
FullMatrices.K3Matrix(:,AddVarPos) = ...
    FullMatrices.K3Matrix(:,AddVarPos) + FullMatrices.K3Matrix(:,RemoveVarPos);

FullMatrices.MMatrix(AddVarPos,:) = ...
    FullMatrices.MMatrix(AddVarPos,:) + FullMatrices.MMatrix(RemoveVarPos,:);
FullMatrices.MMatrix(:,AddVarPos) = ...
    FullMatrices.MMatrix(:,AddVarPos) + FullMatrices.MMatrix(:,RemoveVarPos);

FullMatrices.PMatrix(AddVarPos,:) = ...
    FullMatrices.PMatrix(AddVarPos,:) + FullMatrices.PMatrix(RemoveVarPos,:);
FullMatrices.PMatrix(:,AddVarPos) = ...
    FullMatrices.PMatrix(:,AddVarPos) + FullMatrices.PMatrix(:,RemoveVarPos);

%===============================================================================
% Put to zero elements of the full matrices, which will be removed
%===============================================================================

FullMatrices.K1Matrix(RemoveVarPos,:) = 0;
FullMatrices.K1Matrix(:,RemoveVarPos) = 0;

FullMatrices.K2Matrix(RemoveVarPos,:) = 0;
FullMatrices.K2Matrix(:,RemoveVarPos) = 0;

FullMatrices.K3Matrix(RemoveVarPos,:) = 0;
FullMatrices.K3Matrix(:,RemoveVarPos) = 0;

FullMatrices.MMatrix(RemoveVarPos,:) = 0;
FullMatrices.MMatrix(:,RemoveVarPos) = 0;

FullMatrices.PMatrix(RemoveVarPos,:) = 0;
FullMatrices.PMatrix(:,RemoveVarPos) = 0;

end

