...
function [ResSolid]=St61_Proc_Solid_TE_sp_SAFE(CompStructA,FEMatricesA,ProcTE)
    ...
    while (1>0)
        ...
        node_find_var=find(FEMatricesA.MeshNodes(1,:)==r_find); % & FEMatricesA.MeshNodes(2,:)==z_find);
        node_find=node_find_var(1);
        ...
        if length(rr)>3 && rr_min_prev_var>rr(end-2)
           rr(end-1:end)=[];
           break; 
        end
    ...
    for ieig=ProcTE.neigs
        ...
        if ii_l>1 && ...
          ( strcmpi(CompStructA.Model.DomainType(ii_l-1),'HTTI') && strcmp(CompStructA.Model.DomainType(ii_l),'HTTI') )
        ...
        for ii_n=1:length(FEMatricesA.DNodes{ii_l})
            ...
            if sum(ismember(FEMatricesA.DNodesComp{ii_l},ii_m))==1
            ...
            if sum(ismember(FEMatricesA.DNodesRem{ii_l},ii_m))==1
               ux(ii_n)=0.; uy(ii_n)=0.; uz(ii_n)=0.;
            end
            
            if ii_l>1 && ...
               ( strcmpi(CompStructA.Model.DomainType(ii_l-1),'HTTI') && strcmp(CompStructA.Model.DomainType(ii_l),'HTTI') )
               var_ind=(FEMatricesA.DNodesComp{ii_l-1}==ii_m);
               if sum(var_ind)==1
...
%         for ir=1:Nrr
%           view(45,45); plot3(xx_rf(ir,:),yy_rf(ir,:),abs(ur_rf(ir,:)),'k'); 
%         end
